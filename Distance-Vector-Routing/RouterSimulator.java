...
-DChange        true/false      To activate changing link costs
-DNodes         3, 4, 5         Number of nodes to simulate
-DPoison        true/false      To activate poison reverse
-DSeed          (long)          Random seed
-DTrace         1, 2, 3, 4      Debugging levels

This code is a Java translation of code provided by Kurose and Ross.

Output GUIs added by Ch. Schuba 2007.
Minor updates/reorganizing according to Python changes by C M Bruhner 2021.
...
public class RouterSimulator {
  ...
  public static boolean LINKCHANGES = true;
  public static boolean POISONREVERSE = false;
...
******************************************************************/
  ...
  RouterSimulator()                         /* initialize the simulator */
  {
    ...
    switch(NUM_NODES) {
        ...
        case 5:
            ...
            break;
        default:
            myGUI.println("Panic: Number of nodes outside 3-5\n");
            System.exit(0);
    }
  ...
  void runSimulation(){
	...
	myGUI.println("MAIN: rcv event, t="+
	    ...
	    myGUI.print(" src:"+eventptr.rtpktptr.sourceid);
            ...
            for(int i=0;i<NUM_NODES;i++)
              myGUI.print(" " + eventptr.rtpktptr.mincost[i]);
            myGUI.println();
          }
      }
  ...
  public static void initialize(){
    String prop;
    if((prop=System.getProperty("Change"))!=null)
      RouterSimulator.LINKCHANGES = Boolean.parseBoolean(prop);

    if((prop=System.getProperty("Nodes"))!=null)
      RouterSimulator.NUM_NODES = Integer.parseInt(prop);

    if((prop=System.getProperty("Poison"))!=null)
      RouterSimulator.POISONREVERSE = Boolean.parseBoolean(prop);

    if((prop=System.getProperty("Seed"))!=null)
      RouterSimulator.SEED = Long.parseLong(prop);

    if((prop=System.getProperty("Trace"))!=null)
      RouterSimulator.TRACE = Integer.parseInt(prop);
  }
}
